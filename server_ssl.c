...
int init_database()
{
    ...
    // Create groups and group_members tables if not exist
    char *err_msg = 0;
    const char *groups_sql = "CREATE TABLE IF NOT EXISTS groups (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE);";
    rc = sqlite3_exec(db, groups_sql, 0, 0, &err_msg);
    if (rc != SQLITE_OK)
    {
        fprintf(stderr, "SQL error (groups): %s\n", err_msg);
        sqlite3_free(err_msg);
    }
    const char *members_sql = "CREATE TABLE IF NOT EXISTS group_members (group_id INTEGER, user_id INTEGER, UNIQUE(group_id, user_id));";
    rc = sqlite3_exec(db, members_sql, 0, 0, &err_msg);
    if (rc != SQLITE_OK)
    {
        fprintf(stderr, "SQL error (group_members): %s\n", err_msg);
        sqlite3_free(err_msg);
    }

    // Create messages table
    const char *messages_sql = "CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, sender TEXT, receiver TEXT, group_name TEXT, message TEXT, type INTEGER, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP);";
    rc = sqlite3_exec(db, messages_sql, 0, 0, &err_msg);
    if (rc != SQLITE_OK)
    {
        fprintf(stderr, "SQL error (messages): %s\n", err_msg);
        sqlite3_free(err_msg);
    }
...
// Helper: Create group if not exists, return group id
int get_or_create_group(const char *group_name)
{
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, "INSERT OR IGNORE INTO groups (name) VALUES (?);", -1, &stmt, 0);
    if (rc == SQLITE_OK)
    {
    ...
    int group_id = -1;
    rc = sqlite3_prepare_v2(db, "SELECT id FROM groups WHERE name = ?;", -1, &stmt, 0);
    if (rc == SQLITE_OK)
    {
        sqlite3_bind_text(stmt, 1, group_name, -1, SQLITE_STATIC);
...
int get_user_id(const char *username)
{
    sqlite3_stmt *stmt;
    int user_id = -1;
    int rc = sqlite3_prepare_v2(db, "SELECT id FROM users WHERE username = ?;", -1, &stmt, 0);
    if (rc == SQLITE_OK)
    {
...
void add_user_to_group(int group_id, int user_id)
{
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, "INSERT OR IGNORE INTO group_members (group_id, user_id) VALUES (?, ?);", -1, &stmt, 0);
    if (rc == SQLITE_OK)
    {
...
void remove_user_from_group(int group_id, int user_id)
{
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, "DELETE FROM group_members WHERE group_id = ? AND user_id = ?;", -1, &stmt, 0);
    if (rc == SQLITE_OK)
    {
        sqlite3_bind_int(stmt, 1, group_id);
        sqlite3_bind_int(stmt, 2, user_id);
...
// Helper: List all groups
void list_groups_sqlite(char *buffer, size_t bufsize)
{
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, "SELECT name FROM groups;", -1, &stmt, 0);
    size_t pos = 0;
    int n = snprintf(buffer + pos, bufsize - pos, "=== Groupes actifs ===\n");
    ...
    if (rc == SQLITE_OK)
    {
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            n = snprintf(buffer + pos, bufsize - pos, "- %s\n", name);
            if (n < 0 || (size_t)n >= bufsize - pos)
                break;
            pos += n;
        }
...
// Helper: Store message in SQLite
void store_message(const char *sender_username, const char *receiver_username, const char *group_name, const char *message, int type)
{
    // Map type int to string
    const char *type_str = (type == 0) ? "private" : (type == 1) ? "group"
                                                                 : "server";
    int sender_id = sender_username ? get_user_id(sender_username) : 0;
    int receiver_id = receiver_username ? get_user_id(receiver_username) : 0;
    int group_id = group_name ? get_or_create_group(group_name) : 0;
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, "INSERT INTO messages (sender_id, receiver_id, group_id, message, message_type) VALUES (?, ?, ?, ?, ?);", -1, &stmt, 0);
    if (rc == SQLITE_OK)
    {
        if (sender_id > 0)
        ...
        sqlite3_bind_text(stmt, 5, type_str, -1, SQLITE_STATIC);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
    }
}
...
void handle_command(ClientInfo *client, char *command)
{
    ...
    if (strcmp(token, "/join") == 0)
    {
        ...
        {
            int group_id = get_or_create_group(token);
            int user_id = get_user_id(client->username);
            ...
            {
                snprintf(buffer, sizeof(buffer), "SERVER: Erreur lors de la jointure du groupe.\n");
            }
        }
    ...
    else if (strcmp(token, "/leave") == 0)
    {
        ...
        {
            int group_id = get_or_create_group(client->group);
            int user_id = get_user_id(client->username);
            if (group_id > 0 && user_id > 0)
            {
    ...
    else if (strcmp(token, "/group") == 0)
    {
        ...
        store_message(client->username, NULL, client->group, message, 1); // type 1 = group
    ...
    else if (strcmp(token, "/private") == 0)
    {
        ...
        store_message(client->username, target_user, NULL, message, 0); // type 0 = private
    ...
    {
        list_groups_sqlite(buffer, sizeof(buffer));
        send_to_client(client->ssl, buffer);
    }
    ...
    while ((bytes_received = SSL_read(client->ssl, buffer, BUFFER_SIZE - 1)) > 0)
    {
        ...
        else
        {
            ...
            if (client->group[0] == '\0')
            {
                ...
                store_message(client->username, NULL, NULL, buffer, 2); // type 2 = server/broadcast
            ...
            else
            {
                ...
                store_message(client->username, NULL, client->group, buffer, 1); // type 1 = group
                pthread_mutex_lock(&clients_mutex);
                for (int i = 0; i < MAX_CLIENTS; i++)
                {
                    if (clients[i] && clients[i]->is_authenticated && strcmp(clients[i]->group, client->group) == 0)
